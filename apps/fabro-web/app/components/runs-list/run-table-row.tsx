...
function CreatedByCell({ createdBy }: { createdBy: RunWithStatus["createdBy"] }) {
  const display = principalDisplay(createdBy);
...
export function RunTableRow({
  ...
  return (
    <tr className={`group relative border-b border-line transition-colors last:border-b-0 ${selected ? "bg-overlay/30" : "hover:bg-overlay/40"}`}>
      ...
      {show("created_by") && <CreatedByCell createdBy={run.createdBy} />}
      {show("repo") && (
