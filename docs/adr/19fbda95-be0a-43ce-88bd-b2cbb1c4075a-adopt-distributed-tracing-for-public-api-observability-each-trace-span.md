# Adopt Distributed Tracing for Public API Observability: Each Trace Span
...
1. MUST: Each trace span MUST include standardized attributes: service.name, operation.name, http.method, http.status_code, and error.type (if applicable)

## Policy Block

- MUST Each trace span MUST include standardized attributes: service.name, operation.name, http.method, http.status_code, and error.type (if applicable)
