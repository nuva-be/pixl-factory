# Standardize Server-Sent Events (SSE) for Real-Time Configuration and Runtime State Updates: Route Handlers That
...
1. MUST: Route handlers that expose runtime execution state MUST establish SSE connections with appropriate Content-Type headers (text/event-stream)

## Policy Block

- MUST Route handlers that expose runtime execution state MUST establish SSE connections with appropriate Content-Type headers (text/event-stream)
